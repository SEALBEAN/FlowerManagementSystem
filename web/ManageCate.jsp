...
<html>
    ...
    <body>
        <c:import url="header_loginedAdmin.jsp"></c:import>
            ...
            <c:forEach var="cate" items="${requestScope.orderList}">
                <tr>
                    <td><c:out value="${cate.getOrderID()}"></c:out></td>
                    <td><c:out value="${cate.getOrderDate()}"></c:out></td>
                    <td><c:out value="${cate.getShipDate()}"></c:out></td>
                    <td><c:choose>
                            <c:when test="${cate.getStatus() eq 1}"> processing </c:when>
                            <c:otherwise>canceled</c:otherwise>
                        </c:choose>
                    </td>
                    <td><c:out value="${cate.getAccID()}"></c:out></td>
                        <td>
                        <c:if test="${cate.getStatus() eq 1}">
                            <c:url var="mylink" value="mainController">
                                <c:param name="id" value="${cate.getOrderID()}"></c:param>
